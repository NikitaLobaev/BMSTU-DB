...
func (postRepositoryV3 *PostRepositoryV3) SelectById(id uint64) (*models.Post, error) {
	const query = "SELECT id, post_parent_id, user_nickname, message, is_edited, (SELECT forum_slug FROM thread WHERE thread.id = post.thread_id), thread_id, created FROM post WHERE id = $1"
...
func (postRepositoryV3 *PostRepositoryV3) Update(id uint64, postUpdate *models.PostUpdate) (*models.Post, error) {
	...
	const query = "UPDATE post SET message = $1 FROM (SELECT forum_slug FROM thread WHERE thread.id = (SELECT thread_id FROM post WHERE id = $2)) thread WHERE id = $2 RETURNING id, post_parent_id, user_nickname, message, is_edited, thread.forum_slug, thread_id, created"
	post := new(models.Post)
	var postParentId sql.NullInt64
